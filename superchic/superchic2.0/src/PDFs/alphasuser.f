ccc   Strong coupling constant, for PDFINPUT = USER
      function alphas(qsq)
      implicit double precision(a-z)
      integer iord

      IORD = 0                  ! perturbative order (N^mLO,m=0,1,2,3)
      FR2 = 1.D0                ! ratio of mu_f^2 to mu_r^2
      MUR = 91.1876D0                ! input mu_r in GeV
      ASMUR = 0.135D0             ! input value of alpha_s at mu_r
      MC = 1.4D0                ! charm quark mass
      MB = 4.75D0               ! bottom quark mass
      MT = 1.D10                ! top quark mass

      CALL INITALPHAS(IORD, FR2, MUR, ASMUR, MC, MB, MT)

      alphas=alfas(dsqrt(qsq))

      return
      end

cccc Name of 'alphas' function mmodified to avoid clash with superchic naming scheme - LHL 24/3/15
...
C--   Find the zero of this function using DZEROX.
      ...
      IMPLICIT NONE
      INTEGER IORD
      DOUBLE PRECISION FR2, R0, ASI, MC, MB, MT, MUR, ASMUR, ALFAS
      COMMON / DZEROXcommon / FR2, MUR, ASMUR, MC, MB, MT, R0, IORD

      CALL INITALPHASR0(IORD, FR2, R0, ASI, MC, MB, MT)
      FINDALPHASR0 = ALFAS(MUR) - ASMUR ! solve equal to zero
...
C----------------------------------------------------------------------
      ...
      DOUBLE PRECISION FUNCTION ALFAS(MUR)
      IMPLICIT NONE
