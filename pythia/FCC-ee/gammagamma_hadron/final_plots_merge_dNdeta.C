...
void final_plots_merge_dNdeta(){
 ...
 h_dsdeta350->SetTitle("PYTHIA6 simulation of dN/d#eta for e^{+}e^{-}#rightarrow e^{+}e^{-}/#gamma*#gamma* #rightarrow h^{#pm}, total generated event: 25000");
 ...
 leg->AddEntry(h_dsdeta90,Form("#sqrt{s}=90,  %.2f charged hadrons",total_hadrons[0]),"lep");
 leg->AddEntry(h_dsdeta160,Form("#sqrt{s}=160,  %.2f charged hadrons",total_hadrons[1]),"lep");
 leg->AddEntry(h_dsdeta240,Form("#sqrt{s}=240,  %.2f charged hadrons",total_hadrons[2]),"lep");
 leg->AddEntry(h_dsdeta350,Form("#sqrt{s}=350,  %.2f charged hadrons",total_hadrons[3]),"lep");
 leg->Draw();
cout <<"ciao" << endl;
