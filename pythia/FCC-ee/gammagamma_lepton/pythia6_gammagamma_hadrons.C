...
void pythia6_gammagamma_hadrons( int Nevts = 10000, double sqrts = 90, int MSTP14_val=10) 
{
  ...
  for (int ievt = 0; ievt < Nevts; ievt++) {
    ...
    h_had_per_ev->Fill(n_leptons);

    n_leptons_total += n_leptons;
  ...
  cout << endl << endl << endl <<  "Total number of charged hadrons = " << n_leptons_total << endl;
  cout << "Cross section = " << total_leptons_xsection << "pb  in e+e- --> gamma gamma --> X at sqrt(s) = " << sqrts << " GeV " << endl << endl << endl;

  Double_t N_rate = total_leptons_xsection*Luminosity;
  ...
  file_out << total_leptons_xsection << " // [pb] Cross section for e+e- --> gamma gamma --> X"<< endl;
  ...
  hdNdeta=(TH1F*)hdsigmadeta->Clone();
